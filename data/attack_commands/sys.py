...
class SystemCommands:
    ...
    def command_top(self, command, client_ip):
        ...
        # Prepare the output
        output = ""
        iteration_count = iterations if iterations else 1
        for i in range(iteration_count):
            ...
            header = (
                f"top - {current_time} up {uptime_str},  1 user,  load average: {load_average}\n"
            ...
            process_list = self.generate_process_list(pids)
            output += header + process_list
            if iterations and i < iteration_count - 1:
                output += "\n"
                time.sleep(delay)
        return output
    ...
    def generate_process_list(self, pids):
        ...
        for proc in processes:
            process_list += (
                f"{str(proc['pid']).rjust(5)} {proc['user'].ljust(8)} {proc['pr'].rjust(2)} {proc['ni'].rjust(3)} "
                f"{proc['virt'].rjust(7)} {proc['res'].rjust(7)} {proc['shr'].rjust(7)} {proc['s']}  {proc['cpu'].rjust(4)} "
                f"{proc['mem'].rjust(4)} {proc['time'].rjust(9)} {proc['command']}\n"
            )
        return process_list
    ...
    def command_dmesg(self, command, client_ip):
        ...
        if '-T' in args or '--ctime' in args:
            output = self.format_dmesg_with_timestamps(buffer)
            return output
